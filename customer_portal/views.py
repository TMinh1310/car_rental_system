    ...
    for a in area:
        ...
        for car in vehicles:
            if car.is_available == True:
                vehicle_dictionary = {'name':car.car_name, 'color':car.color, 'id':car.id, 'district':car.area.district, 'capacity':car.capacity, 'description':car.description}
...
def confirm(request):
    ...
    if vehicle.is_available:
        ...
        car_owner.wallet += rent
        car_owner.save()
        try:
            order = Orders(vehicle = vehicle, car_owner = car_owner, user = user, rent=rent, days=days)
            order.save()
        except:
            order = Orders.objects.get(vehicle = vehicle, car_owner = car_owner, user = user, rent=rent, days=days)
    ...
    if orders is not None:
        for o in orders:
            if o.is_complete == False:
                order_dictionary = {'id':o.id,'rent':o.rent, 'vehicle':o.vehicle, 'days':o.days, 'car_owner':o.car_owner}
...
def update_order(request):
    ...
    car_owner.wallet -= int(order.rent)
    car_owner.save()
    order.delete()
    cost_per_day = int(vehicle.capacity)*13
    return render(request, 'customer/confirmation.html', {'vehicle':vehicle}, {'cost_per_day':cost_per_day})
